...
use crate::api::{Futures, Portfolio, API};
...
impl PortfolioAccount {
        ...
        F: Into<f64>,
    {
        ...
        let request = build_signed_request(order, self.recv_window)?;
        self.client
            .post_signed(API::Portfolio(Portfolio::Order), request)
    }
        ...
        S: Into<String>,
    {
        ...
        let request = build_signed_request(parameters, self.recv_window)?;
        self.client
            .get_signed(API::Portfolio(Portfolio::PositionRisk), Some(request))
    }
    ...
    pub fn account_information(&self) -> Result<AccountInformation> {
        ...
        let request = build_signed_request(parameters, self.recv_window)?;
        self.client
            .get_signed(API::Portfolio(Portfolio::Balance), Some(request))
    }

    pub fn account_balance(&self, symbol: &str) -> Result<AccountBalance> {
        let mut parameters = BTreeMap::new();
        parameters.insert("asset".to_string(), symbol.to_string());
